...
IF NOT DEFINED TEST_PARAMETERS (
    ECHO Skipping test operations because no test parameters are defined for build unit "!BUILD_UNIT!".
    EXIT /B 0
)
...
IF NOT DEFINED PACKAGE_PARAMETERS (
    ECHO Skipping package operations because no package parameters are defined for build unit "!BUILD_UNIT!".
    EXIT /B 0
)
...
IF NOT DEFINED PACKAGE_NEV_PARAMETERS (
    ECHO   Skipping package dependency version update operations because no such package parameters are defined for build unit "!BUILD_UNIT!".
    EXIT /B 0
)
...
SET PACKAGE_NEV_BUILD_COMMAND=dotnet build "!WORKSPACE_FOLDER_PATH!\tools\Explicit.NuGet.Versions\Explicit.NuGet.Versions.slnx" --configuration Release

REM ECHO   !PACKAGE_NEV_BUILD_COMMAND!
!PACKAGE_NEV_BUILD_COMMAND! >NUL || EXIT /B 13
...
FOR %%G IN (%PACKAGE_NEV_PARAMETERS%) DO (
    ...
    !PACKAGE_NEV_COMMAND! >NUL || EXIT /B 14
)
